#!/bin/sh
# tys: Read the Word of God in Polish from your terminal
...
show_help() {
	...
	echo
	echo "      /<Search>"
	echo "          All verses that match a pattern"${1
	echo "      <Book>/<Search>"
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data tys.tsv | awk -v cmd=list "$(get_data tys.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data tys.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data tys.awk)" | ${PAGER}
	done
	exit 0
fi

get_data tys.tsv 2>/dev/null | awk -v cmd=ref -v ref="$*" "$(get_data tys.awk)" 2>/dev/null | ${PAGER}
